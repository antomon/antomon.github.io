...
description: "Recent results from Google and emerging approaches developed by Oratomic have significantly refined the quantitative understanding of the quantum threat to public-key cryptography, shifting the discussion from theoretical possibility to engineering feasibility. This article analyzes these developments from first principles, distinguishing between logical and physical qubits, circuit complexity, and error correction overhead, and examining how improvements at each layer translate into concrete resource estimates for executing Shor’s algorithm against elliptic curve cryptography. In particular, Google’s analysis shows that breaking a 256-bit elliptic curve, such as secp256k1, may require on the order of one thousand logical qubits and tens of millions of non-Clifford gates, corresponding to hundreds of thousands of physical qubits under surface-code assumptions, while alternative architectures proposed by Oratomic, based on neutral-atom systems and high-rate error-correcting codes, suggest that the requirement could be reduced to the order of tens of thousands of physical qubits under specific connectivity and noise assumptions. By integrating these results, the article demonstrates that feasibility is governed by the convergence of multiple independent factors, including circuit optimization, error correction efficiency, and hardware scaling, which interact multiplicatively rather than additively. The implications extend beyond cryptocurrencies to the broader digital infrastructure, introducing temporal asymmetries in attack and defense and reframing the problem from one of mathematical security to one of system architecture and governance."
date: "2026-03-31"
...
The central numerical result reported by Google is that solving the elliptic curve discrete logarithm problem for a 256-bit curve, such as **secp256k1**[^quantum-cryptography-edge-feasibility-resource-estimates-architectural-divergence-systemic-risk-secp256k1] used in Bitcoin, requires on the order of:
...
%%| fig-cap: Mapping from Shor’s algorithm to physical resources under surface-code assumptions based on Google’s estimates
%%| fig-alt: Flowchart from Shor algorithm to logical qubits, optimized circuit, Toffoli gates, surface code, physical qubits, and runtime
%%| fig-align: center
%%{init: {"theme": "neo", "look": "handDrawn"}}%%
flowchart LR
