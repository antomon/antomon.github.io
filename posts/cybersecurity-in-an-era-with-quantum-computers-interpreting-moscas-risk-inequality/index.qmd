...
description: "Mosca’s risk inequality reframes the quantum threat as a present-day governance problem: the lifetime of protected assets plus the time required to migrate cryptography must remain shorter than the time until current public-key systems become breakable. This article explains the model and derives practical implications for data classification, cryptographic inventories, signing systems, vendor requirements, and migration planning."
abstract: |
  This article interprets Michele Mosca’s quantum-risk inequality, in which the security lifetime of an asset, $x$, plus the time required to migrate vulnerable systems, $y$, is compared with the expected time until cryptographic collapse, $z$. When $x+y>z$, exposure already exists because information and signed artifacts created today may remain valuable after their cryptographic protection becomes breakable.

  The framework shifts attention from speculative quantum timelines to variables organizations can influence. By classifying assets according to required protection horizons, inventorying cryptographic dependencies, prioritizing long-lived confidentiality and signing risks, and engineering cryptographic agility, enterprises and institutions can reduce migration time and limit the production of durable quantum-vulnerable artifacts.
date: "2019-12-08"
