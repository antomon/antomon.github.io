...
# Function to solve Sudoku puzzles of size n x n
def solve_sudoku(puzzle, n):
  # Create an n x n matrix of integer variables
  X = [[Int(f"x_{i}_{j}") for j in range(n)] for i in range(n)]  # <2>
  
  # Define the solver
  solver = Solver()  # <3>
  
  # Add constraints for cell values between 1 and sqrt(n)
  sqrt_n = int(n**0.5)  # Calculate the square root of n for subgrids
  solver.add([And(X[i][j] >= 1, X[i][j] <= sqrt_n) for i in range(n) for j in range(n)])  # <4>
  
  # Add constraints for the pre-filled cells
  solver.add([X[i][j] == puzzle[i][j] for i in range(n) for j in range(n) if puzzle[i][j] != 0])  # <5>
  
  # Add row constraints: each row must contain distinct values
  solver.add([Distinct(X[i]) for i in range(n)])  # <6>
  
  # Add column constraints: each column must contain distinct values
  solver.add([Distinct([X[i][j] for i in range(n)]) for j in range(n)])  # <7>
  ...
  solver.add([Distinct([X[i + di][j + dj] for di in range(sqrt_n) for dj in range(sqrt_n)])
              for i in range(0, n, sqrt_n) for j in range(0, n, sqrt_n)])  # <8>
  
  # Find all possible solutions
  solutions = []  # List to store all solutions
  while solver.check() == sat:  # <9>
    model = solver.model()
    solution = [[model.evaluate(X[i][j]).as_long() for j in range(n)] for i in range(n)]  # <10>
    solutions.append(solution)  # Store the solution  # <11>

    # Add a constraint to prevent finding the same solution again
    solver.add(Or([X[i][j] != solution[i][j] for i in range(n) for j in range(n)]))  # <12>
  
  return solutions  # Return all solutions  # <13>
...
if __name__ == "__main__":
  ...
  # Solve the Sudoku puzzle for a 4x4 grid
  n = 4  # Size of the grid (4x4)
  solutions = solve_sudoku(puzzle, n)  # <15>
...
1. **Size Flexibility**: The puzzle size `n` is passed as a parameter to the `solve_sudoku()` function. The size `n` can be any square number (like 4, 9, 16), and the `sqrt_n` is calculated to define the size of the subgrids.  # <2>
2. **Cell Value Range**: Constraints ensure that the values in the grid are between `1` and `sqrt_n`. This adjusts the constraints based on the grid size.  # <4>
3. **Multiple Solutions**: The solver continues to find new solutions using a `while` loop until no further solutions are found. After finding a solution, a constraint is added to the solver to ensure that the same solution cannot be found again (`Or(X[i][j] != solution[i][j])`).  # <12>
4. **Example with Multiple Solutions**: A 4x4 Sudoku puzzle is provided, which has more than one valid solution.  # <14>

### Output Example (for the 4x4 puzzle):
```plaintext
Found 2 solution(s):
Solution 1:
[1, 4, 2, 3]
...
This code can now handle different Sudoku grid sizes, find all possible solutions, and print each one.