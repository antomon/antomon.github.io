...
```python
solver.add(X + Y == 10)  # Add constraint that X + Y equals 10
solver.add(X > 0, Y > 0)  # Add constraint that X and Y are greater than 0
```
...
```python
from z3 import Or
solver.add(Or(X == 3, Y == 7))  # X must be 3 or Y must be 7
```
...
```python
if solver.check() == sat:
    model = solver.model()  # Retrieve the solution
...
Here, `sat` indicates that the problem is satisfiable. If a solution is found, `solver.model()` retrieves the values of the variables that meet all the constraints.

In the example above, if the solver finds a solution, it evaluates the variables `X` and `Y` using `model.evaluate()`. For example, if `X = 3` and `Y = 7`, the output will be:
...
If no solution exists, `solver.check()` will return `unsat`, meaning the constraints are unsatisfiable.
...
solver.add(X + Y == 10)
solver.add(X > 0, Y > 0)
solver.add(Or(X == 3, Y == 7))

# Solve the problem
if solver.check() == sat:
    model = solver.model()

    print(f"Solution: X = {model.evaluate(X)}, Y = {model.evaluate(Y)}")
...
- Z3 allows you to add **arithmetic**, **logical**, and **set-theoretic constraints** to your problem. These constraints can involve:
  ...
  ```python
  solver.add(X + 2 * Y == 10)  # Add an arithmetic constraint
  solver.add(And(X > 0, Y < 10))  # Add logical constraints
  ```
...
- Z3 also supports **optimization** over variables, where you can minimize or maximize a given objective function. This is useful in cases where you are not just looking for any solution, but the **best** one (e.g., minimal cost, maximum profit). You can use `Optimize()` instead of `Solver()` to define an optimization problem:
  ...
  opt = Optimize()  # Initialize an optimization solver
  opt.add(X + Y >= 10)
  opt.minimize(X)  # Minimize X

  if opt.check() == sat:
      model = opt.model()

      print(model.evaluate(X), model.evaluate(Y))
  ```
...
- Z3 supports **first-order logic** and can handle **universal** and **existential quantifiers**. You can express statements like "for all" or "there exists" using `ForAll()` and `Exists()`, respectively:

  ```python
  from z3 import ForAll, Exists
  X = Int('X')
  solver.add(ForAll([X], X >= 0))  # X is non-negative for all X
  ```
...
# Add constraints
solver.add(X + Y == 10)  # Arithmetic constraint
solver.add(And(X > 0, Y > 0))  # Logical constraint
solver.add(Or(X == 3, Y == 7))  # Logical disjunction
