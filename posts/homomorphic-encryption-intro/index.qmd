...
sequenceDiagram
    ...
    Note over C: Step 1: Encrypt private data<br>$Enc(m)$
    C->>C: Encrypt message m<br>using private key
    Note over C,S: Step 2: Send encrypted data<br>to the server
    C->>S: $Enc(m)$
    Note over C: Step 3: Query the server<br>with function $f()$
    C->>S: Send query $f()$
    Note over S: Step 4: Homomorphic computation<br>$Eval(f, Enc(m))$
    S->>S: Perform $Eval(f)$ on<br>$Enc(m)$ without decryption
    Note over S,C: Step 5: Return the encrypted result
    S->>C: $Enc(f(m))$
    Note over C: Step 6: Decrypt and recover result<br>$f(m)$
    C->>C: Compute $Dec(Enc(f(m))) = f(m)$
```
