...
%%{init: {'theme': 'forest', 'themeVariables': {'background': '#ffffff'}}}%%

sequenceDiagram
  actor C as Client (C)
  actor S as Server (S)

  C ->> C: Step 1: Encrypt private data<br>$Enc(m)$
  C ->> S: Step 2: Send $$Enc(m)$$ to S
  C ->> S: Step 3: Send query $$f()$$ to S
  S ->> S: Step 4: Perform $$Eval(f)$$ on $$Enc(m)$$
  S ->> C: Step 5: Return $$Enc(f(m))$$
  C ->> C: Step 6: Computes $$Dec(Enc(f(m))) = f(m)$$, and recovers $$f(m)$$
```
