...
<button id="play-button" style="margin-top: 20px; padding: 10px 20px; font-size: 16px;">
  ▶️ Play Audio
</button>

<button id="pause-button" style="margin-top: 20px; padding: 10px 20px; font-size: 16px; display: none;">
  ⏸️ Pause Audio
</button>

<label for="voice-selection" style="margin-left: 20px;">Choose a Voice: </label>
<select id="voice-selection" style="margin-top: 10px;"></select>
...
<script>
  ...
  // Function to read all text after the marker
  function startReading() {
    // Locate the marker and get all text after it
    const marker = document.createComment('start-tts-content'); // Find the HTML comment marker
    const textToRead = getTextAfterMarker(marker);
    ...
    utterance.onend = () => {
      ...
      progressBar.value = 100;  // Mark progress as complete
    };

    // Speech boundary event to track progress
    utterance.onboundary = (event) => {
      if (event.name === 'word') {
        const wordIndex = event.charIndex / textToRead.length;
        progressBar.value = wordIndex * 100;
      }
    };
  ...
  // Helper function to get text after the marker
  function getTextAfterMarker(startElement) {
    let textContent = '';
    let node = startElement.nextSibling; // Start from the next sibling of the marker
...
- Where to find it: Available in [Python’s documentation](https://docs.python.org/3/library/ctypes.html).
