...
Achieving continuous orchestration at scale calls for reimagining both organizational roles and IT architecture. The CARO and VAOP concepts provide a way to structure this reimagining. CARO is a proposed C-level role that governs the interplay of human and artificial agents in the enterprise[^from-S&OP-to-continuous-orchestration-M22025]. VAOP is an envisioned enterprise architecture in which AI agents become first-class _actors_ in business processes, not merely tools. Together, CARO and VAOP enable a company to dynamically allocate work between humans and AI, maintaining control and accountability even as more processes run autonomously. We introduce each concept and explore how they situate human and agent roles in an organization striving for near-continuous planning.
...
Thus digital legibility becomes the substrate of orchestration. Bounded rationality describes the limits of human processing; requisite variety prescribes how much internal diversity is needed; digital legibility ensures that the environment is even perceivable in the first place.
...
$$
the mutual information between derived state and target outcomes $Y$. Better digital legibility raises $\mathcal{U}(t)$.
...
Value emerges from reducing latency $\Lambda(t)$, raising useful information $\mathcal{U}(t)$, and increasing the share of decisions executed autonomously ($\alpha(t)$) and outside legacy ERP ($\beta(t)$), while shrinking routine human interventions $h(t)$. 
...
- Human collaboration. An escalation operator $\mathcal{E}(s_t,u_t)$ defines when humans intervene:
  $$
  u_t' = (1-\mathcal{E})u_t + \mathcal{E}\,\mathcal{H}(s_t,u_t).
  $$

- Value creation. The enterprise value function can be expressed as:
  $$
  V(t) = \mathbb{E}[\text{service}(t)] - \kappa_{\text{cost}}\mathbb{E}[\text{opex}(t)]
          - \kappa_{\text{risk}}\mathbb{E}[\text{risk}(t)]
          - \kappa_{\text{lat}}\Lambda(t).
  $$

This framework formalizes how digital signals, once rendered legible, become the raw material from which adaptive agent behavior emerges. Over time, as $\mathcal{U}(t)$ rises and latency $\Lambda(t)$ falls, the share of autonomous decisions $\alpha(t)$ and extra-ERP logic $\beta(t)$ grows, while routine human interventions $h(t)$ shrink. The result is a system where agents gradually substitute both legacy systems and micro-decisions, leaving humans with a smaller but more strategic field of action.
...
Yet speed without discernment risks oscillation. Eliminating $\Delta$ without recalibrating the control gains leads to overshoot—an enterprise that reacts faster than it can think. Here, humans act as damping agents: overseeing loops, adjusting escalation thresholds $\mathcal{E}(s_t,u_t)$, and deciding when stability outweighs agility.
...
$$
u_t' = (1-\mathcal{E}(s_t,u_t)),u_t ;+; \mathcal{E}(s_t,u_t),\mathcal{H}(s_t,u_t),
$$
...
$$
z^{(k)}_a \in {0,1} \quad\text{with}\quad
z^{(k)}_a =
\begin{cases}
1, & \text{decision acceptable (meets policy/SLAs)} \
0, & \text{decision unacceptable (breach/override/incident)}
\end{cases}
$$

Optionally weight by severity $s^{(k)}_a \in [0,1]$ (e.g., minor tweak $=0.2$, major breach $=1.0$).
...
$$
\gamma_a^{(k)} = \gamma_0 ,\big( \lambda_I , I_a^{(k)} + 1 - \lambda_I \big),
...
&\qquad - \eta^- , \big(1 - z^{(k)}_a\big) , T_a(t), s^{(k)}_a \Big]
...
$$
\beta_a \leftarrow \lambda \beta_a + \sum_{k \in \mathcal{K}_t} \rho^{,t-k} , I^{(k)}_a , (1 - z^{(k)}_a), s^{(k)}_a,
$$
...
As interpretability $I_a(t)$ improves (decision cards, why-logs, counterfactuals), trust calibration accelerates:

$$
I_a(t+1) = I_a(t) + \xi_1 ,\text{fix_gaps} - \xi_2 ,\text{explain_defects}.
$$

Rising $I_a(t)$ increases $\gamma_a^{(k)}$, shrinking $\Pr{\mathcal{E}=1}$ and expanding safe autonomy zones.

Ultimately, these dynamics are not decorative mathematics but a governance equation: a way to quantify and steer trust as the key control variable of hybrid orchestration. The enterprise seeks to maximize autonomy without sacrificing coherence or ethical oversight. This can be expressed as an optimization objective over time:
...
& \Pr{\mathcal{E}_a=1} = f(1 - T_a(t)), \
& \dot{T}_a(t) = \mathcal{F}\big(I_a(t), z_a^{(k)}, s_a^{(k)}\big), \
& \alpha(t) = \sum_a \omega_a [1 - \Pr{\mathcal{E}_a=1}].
\end{aligned}
$$
...
* $R(t)$ and $E(t)$ represent risk exposure and decision errors.
...
\frac{d\alpha(t)}{dt} > 0
\quad\text{while}\quad
R(t) + E(t) \text{ remain bounded below thresholds.}
$$
...
$$
\Pr{\mathcal{E}=1} \rightarrow \epsilon, \quad
T_a(t) \rightarrow T_{\text{stable}}, \quad
\frac{dI_a}{dt} \rightarrow 0,
$$
meaning autonomy stabilizes at a level that is self-correcting, auditable, and ethically transparent.
...
Absorbing variety is not only a technical matter of signals and algorithms but an _organizational act of governance_. Left unmanaged, an ecosystem of agents will splinter: each reacting quickly but not necessarily coherently, creating turbulence rather than taming it.
