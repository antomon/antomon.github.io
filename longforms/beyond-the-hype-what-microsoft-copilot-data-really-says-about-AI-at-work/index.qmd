...
description: "A critical analysis of Microsoft’s Copilot usage study and what observed AI interactions reveal about occupational exposure, task reallocation, and the limits of automation forecasts. The article then extends the evidence from individual activities to enterprise redesign, proposing CARO governance and a Virtual Agent Operational Platform for allocating work across human-led, agent-led, and hybrid operating models."
abstract: |
  Debate about artificial intelligence and employment is still dominated by projections of what models might eventually be capable of doing. This article instead begins with observed use. It examines Microsoft Research’s analysis of approximately 200,000 anonymized Bing Copilot conversations and its attempt to connect real human–AI interactions with the occupational structure defined by the U.S. Department of Labor’s O*NET framework. The resulting AI Applicability Score offers an empirical view of where generative AI is already entering work, which activities it successfully performs, and how much of an occupation’s task structure those activities represent.

  The methodology distinguishes between the user’s goal and the action performed by the AI. This separation makes it possible to differentiate assistance from more automation-like execution and avoids treating every AI-supported activity as equivalent to job substitution. Conversations are mapped to O*NET Intermediate Work Activities, evaluated for frequency, completion, and scope, and then weighted according to their relevance within individual occupations. The article explains the value of this approach while emphasizing that the resulting score is comparative rather than an absolute estimate of jobs automated, workers displaced, or economic value created.

  The results show a highly uneven occupational footprint. Applicability is greatest in work dominated by language, information retrieval, writing, explanation, structured communication, and digitally represented procedures. Interpreters, writers, customer-service workers, sales representatives, and several information-intensive service occupations therefore appear near the top of the ranking. Occupations based primarily on physical manipulation, embodied presence, clinical procedures, machinery, or site-specific safety constraints remain much less affected by a text-oriented assistant. High applicability, however, does not imply the disappearance of the occupation: relational judgment, responsibility, tacit knowledge, negotiation, and contextual interpretation may remain human even where a substantial share of informational work is AI-assisted.

  Comparison with capability-based studies in the United States and Europe shows broad agreement at the level of occupational groups but meaningful divergence at finer granularity. Predictive mappings estimate where AI could be useful; telemetry records where adoption has actually occurred. The gap between the two reflects platform demographics, sector-specific software ecosystems, organizational readiness, regulation, workflow integration, and the availability of suitable data. The analysis therefore argues that no single measure is sufficient. A credible picture of labour-market impact requires triangulation among observed usage, task–capability mappings, vacancy and skills data, sector studies, and eventually direct evidence of process ownership and organizational restructuring.

  The article then moves from occupational exposure to a deeper question: what happens when repeatedly successful AI activities are assembled into persistent business processes? At that point, AI is no longer merely a tool used by an employee. Agents can become operational nodes that own bounded workflows, exchange outputs with other agents, and reduce the human coordination layers surrounding a process. The relevant unit of analysis consequently shifts from an isolated task or job title to an interconnected process network. Exposure must then be measured through process ownership, workflow centrality, compliance-adjusted completion, human rework, and the degree to which an agent influences upstream and downstream roles.

  To govern this transition, the article proposes the Chief Agency Resources Officer, or CARO, as an executive function spanning parts of information technology, human resources, operations, risk, and workforce planning. The CARO would manage human and synthetic agency as one resource portfolio: defining agent roles and autonomy limits, governing lifecycle and performance, planning workforce transitions, establishing accountability, and determining whether a process should remain human-led, become agent-led, or adopt a hybrid structure. Applicability scores become evidence for this allocation process, but not automatic instructions to replace people.

  The associated technical model is the Virtual Agent Operational Platform, or VAOP: an operational fabric in which persistent agents and human orchestrators interact through shared enterprise data, dynamic workflow graphs, AgentOps controls, audit mechanisms, and policy-based security. Its architecture includes common data and state, specialized and coordinating agents, event-driven orchestration, human authority checkpoints, lifecycle management, telemetry, rollback, agent identity and access management, policy-as-code, runtime compliance, and vendor-independent integration. Existing enterprise systems remain important, but increasingly serve as systems of record and state beneath a more adaptive layer of human–agent execution.

  A structured decision framework translates this model into operational governance. Processes are evaluated according to complexity, variability, volume, compliance requirements, data sensitivity, institutional knowledge, error tolerance, continuity, vendor dependence, agent drift, workforce effects, and total lifecycle cost. High-volume and repeatable work with reliable telemetry may support agent ownership; ambiguous, relationship-intensive, low-tolerance work generally remains human-led; mixed processes may benefit from agents performing preparation and validation while humans retain judgment and final authority. Pilot deployment, parallel oversight, measurable quality gates, gradual scaling, and periodic reassessment are treated as mandatory controls rather than implementation details.

  The central conclusion is that generative AI’s near-term effect is better described as the reallocation and recombination of tasks than as uniform job replacement. Its more consequential impact may nevertheless emerge at the organizational level, when task-level capabilities are institutionalized into persistent agent networks and the enterprise begins to allocate work between human and synthetic agency. AI adoption must therefore be governed not only as software deployment, but as operating-model design. Competitive advantage will depend less on acquiring the most capable model than on building the architecture, decision rights, evidence, security controls, and workforce institutions required to deploy agency without losing accountability, resilience, or human judgment.
date: "2025-08-09"
date-modified: "2025-08-10"
categories: [digital transformation, generative ai, enterprise architecture, essay, machine learning, 🇬🇧]
...
audiences: executive, project manager, researcher, subject-matter-expert, technical specialist
...
```{mermaid}
%%| label: user-ai-iwa
%%| fig-cap: Mapping observed conversations to O*NET IWAs
%%| fig-alt: Flow from user–AI conversations through classifiers to O*NET IWA signals and occupational scoring
%%| fig-align: "center"
...
2. **Coverage will shift from frequency to process ownership:**
    ...
    * The Microsoft metric treats _coverage_ as the proportion of an occupation’s activities represented in observed AI use. Once AI is structurally embedded:
...
%%| fig-cap: Allocation of tasks between humans, agents, and hybrids
%%| fig-alt: Decision path for allocating operational tasks to human-led, agent-led, or hybrid modes
%%| fig-align: "center"
%%{init: {"theme": "neo", "look": "handDrawn"}}%%
flowchart TD
...
* **Stage 2 – Agent-orchestrated process islands:**
    ...
    * Strategic implications:
      ...
      * Decision shifts from _buy/build new software_ to _design an AI agent to run this process_.
...
```{mermaid}
%%| label: raci-snapshot
%%| fig-cap: RACI snapshot for hybrid operational governance — R = Responsible, A = Accountable, C = Consulted, I = Informed
%%| fig-alt: RACI diagram with Governance roles above Operations roles in a two-layer structure
%%| fig-align: center
%%{init: {"theme": "neo", "look": "handDrawn"}}}%%
...
The VAOP reshapes the enterprise into **seven interlocking layers**, each with its own technical purpose, governance touchpoints, and role migration trajectory.
By structuring the architecture this way, CARO and their team can systematically decide **where human expertise remains critical, where agents can operate autonomously, and where hybrid oversight is optimal**.
...
Compare this to LLM training: text tokens are a few bytes each, and the *entirety* of GPT-3’s training corpus could fit into **less than one day’s worth of raw sensor capture from a small fleet of robots** (see _Appendix A_ for full derivation).
...
4. **Strategic choice as a continuous process:**
   ...
   * The decision of _human vs. agent_ is no longer a one-off transformation project; it’s a quarterly or even monthly operational adjustment.
...
Dillon, E. W., Jaffe, S., Immorlica, N., & Stanton, C. T. (2025). *Shifting work patterns with generative AI*. arXiv. [DOI](https://doi.org/10.48550/arXiv.2504.11436)

Eloundou, T., Manning, S., Mishkin, P., & Rock, D. (2023). *GPTs are GPTs: An early look at the labor market impact potential of large language models*. arXiv. [DOI](https://doi.org/10.48550/arXiv.2303.10130)
...
Lin, F., Hu, Y., Sheng, P., Wen, C., You, J., & Gao, Y. (2024). *Data scaling laws in imitation learning for robotic manipulation*. arXiv. [DOI](https://doi.org/10.48550/arXiv.2410.18647)

Müller, J. (2025). *In the age of AI, goals will define the org chart*. Workpath Magazine. [URL](https://www.workpath.com/en/magazine/in-the-age-of-ai-goals-will-define-the-org-chart)

Nolan, B. (2025). *AI is already upending the corporate org chart as it flattens the distance between the C-suite and everyone else*. Fortune. [URL](https://fortune.com/2025/08/07/ai-corporate-org-chart-workplace-agents-flattening/)
...
Sartor, S., & Thompson, N. (2024). *Neural scaling laws in robotics*. arXiv. [DOI](https://doi.org/10.48550/arXiv.2405.14005) 

Tomlinson, K., Jaffe, S., Wang, W., Counts, S., & Suri, S. (2025). *Working with AI: Measuring the occupational implications of generative AI* (arXiv:2507.07935). arXiv. [DOI](https://doi.org/10.48550/arXiv.2507.07935)

Walke, H. R., Black, K., Lee, A., Kim, M. J., Du, M., Zheng, C., Vuong, Q., Hansen‑Estruch, P., He, A., Myers, V., Kim, M. J., Du, M., Lee, A., Fang, K., Finn, C., & Levine, S. (2024). *BridgeData V2: A dataset for robot learning at scale*. arXiv. [DOI](https://doi.org/10.48550/arXiv.2308.12952) 
