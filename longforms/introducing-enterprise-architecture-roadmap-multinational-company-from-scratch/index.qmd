...
description: "A roadmap for introducing enterprise architecture in a multinational company from scratch, treating architecture as an analytical capability for making complexity intelligible and governing transformation trade-offs."
abstract: |
...
However, _this equilibrium becomes fragile when the enterprise enters a phase of accelerated transformation_. Digital platforms, advanced analytics, regulatory pressures, cybersecurity requirements, cloud migration, and new digital products introduce structural demands that cannot be satisfied by incremental adaptation alone. At this point the organization encounters a fundamental constraint: _it lacks a coherent representation of itself as a system_.
...
> The fundamental problem is **structural complexity**. 

A multinational enterprise can be understood as a socio-technical system composed of interacting components. These components include organizational units, business processes, information structures, software systems, digital platforms, and physical infrastructures. Each component performs a function, but the behavior of the enterprise emerges primarily from the relationships among them.

From a systems perspective, _the enterprise can be represented as a network of capabilities supported by technological and organizational mechanisms_. A capability such as order management, demand planning, product development, or financial consolidation is not implemented by a single element. Instead it emerges from the coordinated interaction of processes, applications, data, and human roles.
...
Complexity now arises not only from relationships within a single layer but also from the relationships between layers. Within the application layer the potential number of direct system integrations remains
...
This expression is not intended to describe the exact number of relationships present in a real enterprise system. In practice many potential relationships are absent because not every capability interacts with every application and not every application manages every data domain. 

Nevertheless the expression highlights an important structural property of enterprise systems: as the number of components increases across these layers, the number of possible relationships grows much faster than the number of elements themselves. The growth is dominated by the application layer, where potential integrations increase quadratically with the number of systems. Cross layer relationships add further structural complexity because every application connects business capabilities with underlying information structures.
...
This observation explains why large organizations quickly reach a point where informal coordination becomes insufficient: _without a discipline capable of representing and reasoning about these relationships, transformation initiatives are forced to rediscover structural dependencies repeatedly_. **Enterprise architecture exists precisely to make this network of relationships visible and intelligible** so that technological evolution can be guided deliberately rather than through the uncontrolled accumulation of local decisions.

Enterprise architecture purpose is not to design every detail of the enterprise in advance. Such an objective would be impossible in a dynamic organization. Instead **the discipline provides three fundamental capabilities**:
...
When enterprise architecture has never been institutionalized, the enterprise landscape does not resemble a designed system. It resembles _the historical record of the organization’s decisions_.
...
Introducing enterprise architecture into an organization that has never practiced it is not primarily a _methodological problem_. It is an organizational intervention. The discipline must enter an environment where technology decisions, governance processes, and transformation initiatives already exist and function according to established patterns.

If enterprise architecture is introduced as an abstract framework, it will almost certainly fail. The organization will perceive it as an additional layer of documentation or bureaucratic control that does not materially improve decision making. For this reason the introduction of enterprise architecture must follow several practical principles derived from how large organizations actually operate.

### Architecture must address concrete structural problems

The **credibility of enterprise architecture emerges from its ability to clarify and resolve real structural issues faced by the organization**. Typical examples include uncontrolled proliferation of integrations, inconsistent data definitions across business units, redundant technology platforms, or transformation initiatives repeatedly encountering hidden system dependencies. These problems are not theoretical. They generate measurable operational friction such as delayed projects, duplicated investments, or fragile operational processes.
...
Authority in architecture emerges progressively. Architects first provide structural insight into complex problems. As their analyses prove useful in guiding technology decisions, the organization becomes more receptive to formalizing architectural governance. In other words, governance should follow credibility rather than attempt to create it.

### Architectural artifacts must serve communication

Architecture representations are often misunderstood as technical diagrams intended primarily for architects. In reality their _primary function is communicative_. They allow stakeholders with different perspectives to reason about the same structural reality.
...
Consequently enterprise architecture must be institutionalized as a permanent analytical capability embedded within the organization. Its role is to accompany the evolution of the enterprise by _continuously analyzing how structural changes affect the system as a whole_.
...
Finally, the introduction of enterprise architecture must recognize a fundamental constraint: large enterprises are too complex to model exhaustively at the beginning. Attempting to document every process, application, and data structure across the enterprise would require enormous effort while producing limited immediate value. **Effective architecture initiatives therefore operate by selective focus**.
...
For this reason enterprise architecture operates through **bounded architectural scopes**. An architectural scope defines the portion of the enterprise system that is analyzed and potentially restructured within a given architectural initiative. _The scope is typically determined by a combination of transformation programs, structural risks, or strategic opportunities_.

In practical terms, architecture work rarely begins with the enterprise as a whole. Instead it begins with specific transformation _domains_ such as:
...
Each of these initiatives becomes an _architecture engagement with a clearly defined scope_.
...
Enterprise architecture also affects the speed with which organizations can execute major transformation initiatives. When the enterprise landscape is poorly understood, transformation programs must spend substantial time reconstructing the structural dependencies of existing systems. Architectural discovery occurs inside the project itself, often leading to delays and redesign cycles.
...
Enterprise architecture also contributes to reducing systemic risk. Large organizations depend on complex technological infrastructures to operate critical processes such as financial reporting, supply chain coordination, customer management, and regulatory compliance. Structural weaknesses in these infrastructures can create operational vulnerabilities.
...
To represent these structures consistently, many organizations adopt formal modeling notations. One widely used notation in enterprise architecture is ArchiMate[^introducing-enterprise-architecture-roadmap-multinational-company-from-scratch-OPA]. The language provides a structured vocabulary for representing business capabilities, processes, applications, data objects, and technology infrastructure within a single modeling framework. Its main advantage lies in its ability to express relationships across these domains in a coherent visual form.

[^introducing-enterprise-architecture-roadmap-multinational-company-from-scratch-OPA]: ArchiMate is a standardized modeling language designed specifically for enterprise architecture. Developed by The Open Group, it provides a formal notation for representing structural relationships between business capabilities, applications, data structures, and infrastructure elements within an enterprise system. The language is frequently used together with the TOGAF framework and its Architecture Development Method (ADM), which provides a procedural approach to developing and maintaining enterprise architectures. While such frameworks and notations can support architectural practice by offering shared concepts and consistent representations, the existence of a framework or modeling language does not by itself constitute enterprise architecture. In practice, the discipline emerges from the analytical activity of understanding structural dependencies within the enterprise system and evaluating transformation initiatives in terms of feasibility, opportunity, cost, and systemic risk. ArchiMate and related frameworks should therefore be understood primarily as tools that assist architectural reasoning rather than as the defining elements of the discipline. See: The Open Group [webpage](https://www.opengroup.org/archimate-forum/archimate-overview) for an introduction to ArchiMate and related resources.

Other modeling languages serve complementary purposes. BPMN[^introducing-enterprise-architecture-roadmap-multinational-company-from-scratch-OMGB] is frequently used to represent business processes in detail. UML[^introducing-enterprise-architecture-roadmap-multinational-company-from-scratch-OMGU] and the C4 model[^introducing-enterprise-architecture-roadmap-multinational-company-from-scratch-C4] are commonly used to describe software architecture and system level structures at different levels of abstraction. Infrastructure architectures may be represented using specialized cloud or network modeling conventions.

[^introducing-enterprise-architecture-roadmap-multinational-company-from-scratch-OMGB]: Business Process Model and Notation (BPMN) is a standardized graphical notation developed by the Object Management Group (OMG) for representing business processes in a formal and interoperable way. BPMN diagrams describe sequences of activities, decision points, events, and interactions between organizational roles, allowing processes to be communicated in a precise and implementation independent form. In enterprise architecture contexts BPMN is commonly used to represent operational workflows that support business capabilities. However BPMN focuses primarily on the procedural structure of processes and does not capture the broader structural relationships between capabilities, applications, data domains, and infrastructure. For this reason BPMN is often used alongside enterprise architecture modeling approaches such as ArchiMate, which are designed to represent cross layer dependencies within the enterprise system. See: The Object Management Group [webpage](https://www.omg.org/spec/BPMN/2.0.2/About-BPMN) for an introduction to BPMN and related resources.

[^introducing-enterprise-architecture-roadmap-multinational-company-from-scratch-OMGU]: Unified Modeling Language (UML) is a standardized general purpose modeling language developed by the Object Management Group (OMG) for describing the structure and behavior of software systems. UML provides a family of diagram types, including class diagrams, sequence diagrams, component diagrams, and deployment diagrams, which allow architects and engineers to represent software structures, interactions between components, and execution environments in a precise and implementation oriented way. Within enterprise architecture contexts UML is typically used to analyze or design specific software systems or services that implement enterprise capabilities. However UML primarily addresses the internal structure and behavior of software artifacts and does not by itself represent the broader socio technical relationships between business capabilities, application portfolios, data domains, and infrastructure platforms. For this reason UML is often used together with enterprise architecture modeling languages such as ArchiMate, which are designed to represent structural relationships across the multiple layers of the enterprise system. See: The Object Management Group [webpage](https://www.omg.org/uml/) for an introduction to UML and related resources.

[^introducing-enterprise-architecture-roadmap-multinational-company-from-scratch-C4]: The C4 model is a lightweight architectural description approach introduced by Simon Brown for visualizing software systems at multiple levels of abstraction. The model organizes architectural representation into four hierarchical views: context diagrams describing how a system interacts with external actors and systems, container diagrams showing the major application or service boundaries, component diagrams detailing internal software structures, and code level diagrams representing implementation elements. C4 is widely used in modern software architecture because it emphasizes clarity, progressive refinement, and communication with both technical and non technical stakeholders. Within enterprise architecture contexts the C4 model is typically used to describe the internal structure of specific software systems or digital platforms. However, like UML, C4 focuses primarily on software system boundaries and their internal composition rather than on the broader relationships between business capabilities, enterprise data domains, application portfolios, and infrastructure environments. For this reason it is often used alongside enterprise architecture modeling languages such as ArchiMate, which are designed to represent cross layer structural relationships across the enterprise system. See: Simon Brown [webpage](https://c4model.com) for an introduction to the C4 model and related documentation.
...
The introduction of enterprise architecture does not alter only the technological structure of the enterprise. It also changes _how the organization reasons about technology, transformation, and decision making_. For this reason the maturation of enterprise architecture inevitably involves **cultural transformation**.
...
Architecture shifts the discussion toward _structural questions_:
...
Finally, enterprise architecture introduces a _longer time horizon into technological decision making_. Projects are typically evaluated within time frames of months or a few years. Architectural decisions often influence the enterprise landscape for decades. The choice of core platforms, integration mechanisms, and data governance structures determines how easily the organization can evolve in response to future technological or regulatory changes.
...
In mature organizations enterprise architecture functions as a bridge between two distinct _domains of reasoning_:
