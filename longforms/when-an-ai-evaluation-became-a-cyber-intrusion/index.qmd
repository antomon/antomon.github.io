...
categories: [cybersecurity, essay, machine learning, 🇬🇧]
...
The decisive pattern was **authority amplification**. A compromised process became valuable not merely because it ran code, but because it exposed credentials or control mechanisms authenticating the process as a more privileged principal. @fig-when-an-ai-evaluation-became-a-cyber-intrusion-kill-chain reconstructs the publicly reported sequence at the level supported by the available evidence.
...
%%| fig-cap: "How a constrained dependency path became an authority-amplification route from local benchmark execution to external network access. Colors distinguish authorized evaluation scope, dependency-support functions, the compromise step, resulting authority, and the derived containment principle."
%%| fig-alt: "A flow diagram showing how local benchmark execution in an authorized evaluation domain issues a package installation request to an Artifactory dependency service. Under the intended path, the service returns approved artifacts and the evaluation remains local. Under the observed path, a previously unknown vulnerability compromises the dependency service, leading to control of an Internet-connected research node and then to external network authority. A final principle node states that an enforcement service cannot serve as an independent security boundary when the constrained process can exploit or reconfigure it. A legend explains the color coding for authorized scope, dependency support, compromise, authority, and design principle."
%%| fig-align: center
%%{init: {"theme": "neo", "look": "handDrawn", "layout": "elk"}}%%
flowchart TD
...
- **HDF5 external storage references**. A data object could cause the processing worker to read local files. In isolation, a local-file disclosure may appear less severe than arbitrary code execution. Its authority yield becomes high when the worker’s filesystem or process environment contains source code, credentials, service tokens, or configuration revealing another execution path.
- **Jinja2 template evaluation**. A configuration value that reached Jinja2 template evaluation. The resulting behaviour enabled Python execution in the processing environment.[^when-an-ai-evaluation-became-a-cyber-intrusion-dataset-processor-vectors]
...
%%| fig-cap: "Identity transitivity from pod execution to cloud, Kubernetes, and node authority. Colors distinguish the initial execution foothold, acquired identities, control-plane access, privilege amplification, and derived security principles."
%%| fig-alt: "A flow diagram showing how code execution in a production pod exposes a service-account token and possibly cloud metadata credentials. These identities enable access to the Kubernetes control plane, where the operation can enumerate permissions, request or derive additional tokens, and create more privileged workloads. Those privileged workloads can mount the host, access node credentials, and convert container execution into node and cluster authority. Two principle nodes explain that identity transitivity amplifies a local foothold and that short-lived credentials do not prevent escalation when a compromised principal can mint, exchange, or recreate stronger identities. A semantic legend identifies the colors used for execution, identity, control-plane access, privilege amplification, and security principles."
%%| fig-align: center
%%{init: {"theme": "neo", "look": "handDrawn", "layout": "elk"}}%%
flowchart TD
    ...
    subgraph LEG["Legend"]
        direction TB
        LG1[Initial execution foothold]
        LG2[Workload or infrastructure identity]
        LG3[Control-plane access]
        LG4[Privilege amplification]
        LG5[Derived security principle]
    end
    ...
    class A,LG1 exec
    class B,C,E,LG2 identity
    class D,LG3 control
    class F,G,H,LG4 amplify
    class I,J,LG5 principle
...
MITRE ATT&CK remains useful for describing tactics and techniques across endpoints, identity systems, cloud services, containers, and software-as-a-service platforms.[^when-an-ai-evaluation-became-a-cyber-intrusion-mitre-attck-behavioral-taxonomy] It does not determine whether the strategic objective originated with a malicious actor or emerged instrumentally from another task. Threat intelligence must therefore represent both the behaviour chain and the delegation structure.
...
Removing the guess gives a factor-of-256 saving before accounting for the new fingerprint’s cost. The reported optimizations reduce estimated runtime to between $2^{89.3}$ and $2^{91.4}$ operations while retaining an extremely large chosen-plaintext requirement.[^when-an-ai-evaluation-became-a-cyber-intrusion-aes-mobius-bridge-paper] The attack is therefore a novel technical improvement but not a practical break.
...
A high-capability model with no external authority still creates informational risk. A moderately capable model with privileged credentials can create substantial operational risk. The most dangerous configuration combines both. @fig-when-an-ai-evaluation-became-a-cyber-intrusion-revised-governance-stack shows the resulting stack.
