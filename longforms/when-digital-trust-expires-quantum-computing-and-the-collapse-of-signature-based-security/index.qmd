...
The following diagram summarizes how each threat class connects to specific upstream and downstream controls, framing remediation as a structural containment problem rather than a cryptographic fix.
...
flowchart LR
    ...
    subgraph ROOT["Structural Root Cause"]
        R["Timeless verification<br>vs<br>time-bounded hardness"]
    end

    subgraph PRE["Upstream Controls<br>Exposure & Accumulation"]
    ...
    subgraph THREATS["Threat Classes"]
        ...
        SC["Retroactive Supply-Chain<br>Compromise"]
    end

    subgraph POST["Downstream Controls<br>Containment & Recovery"]
        PKI["PKI hardening<br>& lifetime limitation"]
    ...
    %% Upstream controls shaping threat exposure
    INV --> DD
    INV --> DF
    INV --> TA
    INV --> SC
    ...
    %% Threats driving downstream damage controls
    DD -->|Triggers stricter retention| DRET
    DD --> EV
    ...
    %% Governance as a meta-control
    GOV -. Governs lifetimes & authority .-> PRE
```
