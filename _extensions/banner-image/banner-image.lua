...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var metaBlocks = Array.from(titleBlock.querySelectorAll(".quarto-title-meta"));
  ...
  function headingText(meta) {
    var h = meta.querySelector(".quarto-title-meta-heading");
    return h ? h.textContent.trim().toLowerCase() : "";
  }

  metaBlocks.forEach(function(meta) {
    var h = headingText(meta);

    if (h === "author" || h === "affiliation") {
      row1.appendChild(meta);
    } else if (h === "published" || h === "modified" || h === "reading time") {
