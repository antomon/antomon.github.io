...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function makeMetaBlock(labelText, valueNode) {
    ...
    contents.appendChild(valueNode);
  ...
  function normalizeAuthorBlock(block) {
    ...
    if (children.length < 4) return [block];
    ...
    return [authorMeta, affiliationMeta].filter(Boolean);
  }
    ...
    items.forEach(function(item) {
      ...
      if (contents.children.length === 1) {
        ...
        while (contents.firstChild) {
          valueNode.appendChild(contents.firstChild);
        }
      }
